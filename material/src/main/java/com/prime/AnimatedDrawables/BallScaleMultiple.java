...
public class BallScaleMultiple extends AbstractAnimatedDrawable {

    float[] mScales = new float[]{1, 1, 1};
    int[] mAlphas = new int[]{255, 255, 255};
    ...
    public void draw(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            canvas.scale(mScales[i], mScales[i], getWidth() / 2, getHeight() / 2);
            canvas.drawCircle(getWidth() / 2,
                    getHeight() / 2,
                    getWidth() / 2 - circleSpacing,
                    paint);
        }
    }
